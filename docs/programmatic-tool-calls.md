...
- **`run_code`** MCP tool. When the flag is **on** it is exposed **instead of**
  `run_tool`; when **off** nothing changes.
...
description states): **bare assignment persists** (`bugs = await PTC_X()` → lives
on the context global across calls); **`const`/`let` are local to one call.**
