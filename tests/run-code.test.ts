...
describe("handleRunCode", () => {
  ...
  it("overflows a large return value to a file (no overflow boolean field)", async () => {
    ...
    expect(env.overflow).toBeUndefined(); // derivable field dropped
    expect(env.value.__overflow).toBe(true);
    expect(env.value.shape).toContain("blob: string");
    expect(env.value.note).toBeUndefined(); // dropped; the hint covers it
  ...
  it("persists bare-assigned vars; session present only when fresh; reset clears", async () => {
    ...
    expect(third.error.location).toMatch(/cell line/); // line number surfaced
  });
  ...
  it("hints + clear error on unknown PTC_ tool", async () => {
    const env = parseEnvelope(
  ...
  describe("failure visibility", () => {
    it("a failed tool call THROWS (PTC_<tool> failed: <reason>) → ok:false", async () => {
      ...
      // location is resolved even though the throw originates in the host realm
      // (the binding stamps the cell-side call stack onto __ptcSite).
      expect(env.error.location).toBe("cell line 1");
      expect(env.hints.join(" ")).toMatch(/try\/catch/); // failure-path hint present
      expect(env.ledger).toBeUndefined(); // no ledger
    });

    it("error.location points at the FAILING call line in a multi-line cell", async () => {
      const env = parseEnvelope(
        (await handleRunCode(clientFailing("boom"), makeServer(false), dir, {
          reset: true,
          // blank, comment, then the failing call on cell line 3
          code: `let x = 1;\n// nothing\nconst r = await PTC_DEMO_SEARCH({});\nreturn "unreached";`,
        })) as never,
      );
      expect(env.ok).toBe(false);
      expect(env.error.location).toBe("cell line 3"); // guards the userLineOffset math
    });
    ...
    it("uncaught throw mid-batch: prior bare-assigned state persists, no rollback", async () => {
      ...
      expect(first.hints.join(" ")).toMatch(/persist/); // promises vars survive the throw
