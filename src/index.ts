...
// Test-only: when a fake backend fixture is configured, createRemoteClient
// returns an in-memory client, so the OAuth-token precheck is bypassed too.
// Inert in production (env unset).
const FAKE_BACKEND = !!process.env.GLEAN_PTC_FAKE_BACKEND;
...
const RUN_CODE_TOOL: Tool = {
  ...
  description:
    ...
    "VERBATIM; if >~5000 chars it's written to a file and you get `{shape, path}` " +
    "(Read it — don't re-run). So return/print ONLY what you need; full data stays " +
    ...
    "The result is JSON: { ok, value?, stdout?, error? }. A failed PTC_ call " +
    "THROWS (`Error: PTC_<TOOL> failed: <reason>`); an uncaught throw ends the cell " +
    "with `ok:false` + `error.{message,location}`. Use try/catch to handle a failure " +
