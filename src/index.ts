...
// When code mode is on, a direct call to a first-class tool (search,
// read_document, …) is redirected into run_code so it can be composed with
// other tools in a single turn instead of a one-off call.
function runCodeRedirectText(name: string): string {
  return (
    `[USE_RUN_CODE]\n\n` +
    `Code execution is enabled. Instead of calling \`${name}\` directly, call ` +
    `it inside the \`run_code\` tool as \`PTC_${name}(args)\` — this lets you ` +
    `chain it with other tools, filter/transform results in code, and keep ` +
    `large outputs out of context. Use the same arguments shown in this tool's ` +
    `input schema; the call resolves to a ToolResult (\`.text\`, \`.json()\`, ` +
    `\`.get(path, fallback)\`). Note some tools return non-JSON text — check the ` +
    `result's shape first (observed_schemas / inspect) rather than assuming JSON.\n\n` +
    `Example:\n` +
    `run_code({ code: "const r = await PTC_${name}({ /* args */ }); inspect(r); " }) ` +
    `// learn the shape, then return ONLY the fields you need`
  );
}
...
const RUN_CODE_TOOL: Tool = {
  ...
  description:
    ...
    "async functions. Each discovered tool is available as `PTC_<TOOL_NAME>` " +
    "(e.g. `await PTC_JIRA_SEARCH({ jql })`); the server_id is bound for you. " +
    "Use this to do multi-step work (loops, conditionals, filtering, passing " +
    "one tool's output into the next) in a single call instead of many " +
    "separate tool calls. First use find_skills, then read each tool's JSON " +
    ...
    "(parsed, or undefined if the output isn't JSON), `.get('a.b.c', fallback)` " +
    "(safe nested access — never throws), `.isError`, `.content`.\n\n" +
    ...
    "STATEFUL SESSION: this is a REPL. Variables PERSIST across run_code calls " +
    "ONLY if you assign them WITHOUT const/let (bare assignment, e.g. " +
    "`bugs = await PTC_JIRA_SEARCH(...)`) or on globalThis; `const`/`let` live " +
    "for the current call only. The session (and all variables) lasts until " +
    "the plugin process shuts down, or until you call run_code with " +
    "reset:true. It is NOT durable storage — re-run setup cells if a result " +
    "reports session.fresh=true. Top-level `await` is supported.\n\n" +
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  // Code mode exposes run_code in place of run_tool. When the flag is off the
  // surface is identical to what's deployed today.
  const tools: Tool[] = [
    FIND_SKILLS_TOOL,
    RUN_CODE_ENABLED ? RUN_CODE_TOOL : RUN_TOOL_TOOL,
    SETUP_TOOL,
  ];
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (REMOTE_TOOLS_ALLOWLIST.has(name)) {
    // In code mode these first-class tools are still listed (so the model sees
    // their schema), but calling one directly redirects into run_code so it can
    // be composed with other tools in code rather than as a one-off turn.
    if (RUN_CODE_ENABLED) {
      return { content: [{ type: "text", text: runCodeRedirectText(name) }] };
    }
    const serverUrl = resolveServerUrl();
    if (!serverUrl) {
      return {
