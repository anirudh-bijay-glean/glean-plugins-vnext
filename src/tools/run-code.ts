...
// lives for the lifetime of the plugin process. Variables the model assigns
// without const/let (bare assignment) attach to the context global and persist
// across run_code calls until the process exits or run_code({reset:true}).
// No TTL / LRU / heap eviction — the host owns the process lifecycle.
// ---------------------------------------------------------------------------
...
// data itself. To get actual values, return/print the specific fields you need.
globalThis.inspect = (x) => { const s = __ptcShape(__norm(x)); __ptcPrint(s); return s; };
...
export async function handleRunCode(
  ...
  try {
    ...
    if (ctxFresh) {
      hints.push(
        "Fresh session. Variables you assign WITHOUT const/let (e.g. `bugs = await PTC_X()`) " +
          "persist across run_code calls until the plugin process exits or you pass reset:true.",
      );
    }
